...
describe('notices API', () => {
  ...
  describe('getNotices', () => {
    it('calls GET /api/notices with no params by default', async () => {
      ...
      expect(apiClient.get).toHaveBeenCalledWith('/api/notices', undefined)
    })
    ...
    it('appends search_param when provided', async () => {
      ...
      expect(apiClient.get).toHaveBeenCalledWith('/api/notices?search_param=hello', undefined)
    })
    ...
    it('appends page and size when provided', async () => {
      ...
      expect(apiClient.get).toHaveBeenCalledWith('/api/notices?page=2&size=5', undefined)
    })
  ...
  describe('getNotice', () => {
    it('calls GET /api/notices/{id} without a token', async () => {
      vi.mocked(apiClient.get).mockResolvedValue(mockNotice)

      await getNotice(1)

      expect(apiClient.get).toHaveBeenCalledWith('/api/notices/1', undefined)
    })
  ...
  describe('createNotice', () => {
    it('calls POST /api/notices with body and token', async () => {
      vi.mocked(apiClient.post).mockResolvedValue(mockNotice)
      const data = { userId: 1, title: 'New Notice', markdownContent: '# New', categoryId: 1 }

      await createNotice(data, 'my-token')

      expect(apiClient.post).toHaveBeenCalledWith('/api/notices', data, 'my-token')
    })
    ...
    it('returns the created notice', async () => {
      ...
      const result = await createNotice(
        { userId: 1, title: 'New Notice', markdownContent: '# New', categoryId: 1 },
        'token',
      )
    ...
    it('propagates ApiError on 401', async () => {
      ...
      await expect(
        createNotice({ userId: 1, title: 'x', markdownContent: 'x', categoryId: 1 }, 'bad'),
      ).rejects.toThrow(ApiError)
    })
  })

  describe('updateNotice', () => {
    it('calls PUT /api/notices/{id} with body and token', async () => {
      vi.mocked(apiClient.put).mockResolvedValue(mockNotice)
      const data = { title: 'Updated', markdownContent: '# Updated', categoryId: 2 }

      await updateNotice(1, data, 'my-token')

      expect(apiClient.put).toHaveBeenCalledWith('/api/notices/1', data, 'my-token')
    })
    ...
    it('returns the updated notice', async () => {
      ...
      const result = await updateNotice(1, { title: 'Updated', markdownContent: '# U', categoryId: 2 }, 'token')
  ...
  describe('deleteNotice', () => {
    it('calls DELETE /api/notices/{id} with token', async () => {
      vi.mocked(apiClient.delete).mockResolvedValue(undefined)

      await deleteNotice(1, 'my-token')

      expect(apiClient.delete).toHaveBeenCalledWith('/api/notices/1', 'my-token')
    })

    it('returns void on success', async () => {
      vi.mocked(apiClient.delete).mockResolvedValue(undefined)

      const result = await deleteNotice(1, 'token')
    ...
    it('propagates ApiError on 403', async () => {
      ...
      await expect(deleteNotice(1, 'bad')).rejects.toThrow(ApiError)
    })
  })
})
